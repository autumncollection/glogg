...
module GLogg
  ...
  def self.ini path=nil, level=nil, variable=nil
    @@log_to_variable = false
    if level
  ...
  def self.log_to_variable?
    @@log_to_variable
  end
  def self.set_log_to_variable log_to_variable
    @@log_to_variable = log_to_variable
    @@log_message = log_to_variable? ? '' : nil
    @@log_to_variable
  end
  ...
  @@log_msg_io_proc = proc do |level, message|
    if level <= @@LOG_LVL
      msg = assemble_message(level, message)
      log_to_variable? ? @@log_message += msg : @@LOG_PATH.write(msg)
    end
    true
  end
